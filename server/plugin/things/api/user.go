...
func (d *UserApi)GetUserCoreList(c *gin.Context) {
	...
	ret,err := Get(fmt.Sprintf("%s/backgrand/user/userCoreList?page=%d&pageSize=%d",
...
func (d *UserApi)GetUserInfos(c *gin.Context) {
	...
	global.GVA_LOG.Error(fmt.Sprintf("pageInfo:%+v",pageInfo))
	ret,err := Post(fmt.Sprintf("%s/backgrand/user/userInfos", global.GVA_CONFIG.Webapi.Addr),
